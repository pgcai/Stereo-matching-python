    ...
    if position == "left":
        ...
        for disparity in range(D // 2**k):
            if disparity == 0:
                ...
                left_move = torch.cat((zero_padding, candidate), 3)

                if method == "subtract":
                    """ subtract method"""
                    leftMinusRightMove = origin - left_move[:, :, :, :origin.shape[3]]
                else:
                    """ concat mathod """
                    leftMinusRightMove = torch.cat((origin, left_move[:, :, :, :origin.shape[3]]), 1)  # concat the channels
