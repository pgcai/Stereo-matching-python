...
class myImageFloder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        disp_L= left.replace('frames_finalpass', 'disparity').replace('.png', '.pfm')
        ...
        if self.training:  
           ...
           processed = preprocess.get_transform(augment=False)
           left_img = processed(np.array(left_img, dtype=np.float32))
           right_img = processed(np.array(right_img, dtype=np.float32))
