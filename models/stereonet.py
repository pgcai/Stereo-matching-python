...
class MetricBlock(nn.Module):
    def __init__(self, in_channel, out_channel, stride = 1):
        ...
        # self.bn1 = DomainNorm(out_channel)
...
if __name__=='__main__':
    ...
    onnx_path = f"./stereonet_576_960.onnx"

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  #

    cost_volume_method = "subtract"
    model = StereoNet(batch_size=1, cost_volume_method=cost_volume_method).to(device)
    ones = torch.ones((1, 3, 576, 960), dtype=torch.float).to(device)
    left, right = ones.to(device), ones.to(device)
    ...
    save_path = onnx_path.replace('.onnx', '_sim.onnx')
